...
const Game = (() => {
  ...
  function renderPlayers() {
    ...
    el.innerHTML = state.players.map(p => {
      ...
      return `<div class="player-card ${isCur?'current':''} ${!p.alive?'dead':''} ${isMe?'mine':''}" style="--pc:${p.char.color}">
        ...
        <div class="pc-stats">
          ${miniStat('⚡',p.stats.speed,p.char.stats.speed)}
          ${miniStat('💪',p.stats.might,p.char.stats.might)}
          ${miniStat('🧠',p.stats.sanity,p.char.stats.sanity)}
          ${miniStat('📚',p.stats.knowledge,p.char.stats.knowledge)}
        </div>
        ${p.items?.length ? `<div class="pc-items">${p.items.map(i=>`<span class="item-tag ${i.isArtifact?'artifact':''}" onclick="Game.showCard(${JSON.stringify(i).replace(/"/g,"'")})" title="${i.name}：${i.effectDesc||''}">${i.isArtifact?'✨':''} ${i.name}</span>`).join('')}</div>` : ''}
      ...
      for (let x=0;x<=6;x++) {
        ...
        if (room) {
          html+=`<div class="cell room-cell ${isHere?'here':''} ${clickable?'valid-move':''} ${room.special==='ritual_target'?'ritual':''}"
                      data-x="${x}" data-y="${y}" onclick="${clickable?`Game.move('${currentFloor}',${x},${y})`:''}">
          ...
          html+=`<div class="cell explore-cell ${clickable?'valid-move':''}"
                      data-x="${x}" data-y="${y}" onclick="${clickable?`Game.move('${currentFloor}',${x},${y})`:''}">
            <div class="explore-q">?</div>
          </div>`;
        } else {
  ...
  // ── Action Console ─────────────────────────────────────────────────────────
  // v3 核心变化：
  //   - 没有"掷移动骰"按钮，移动步数直接等于速度属性
  //   - 摸牌/检定/凶兆检定后，服务端自动切换回合
  //   - 攻击按钮只在：鬼魂降临后 + 本回合未攻击 + 无待处理事项 + 有同格目标 时显示
  //   - "结束回合"按钮：无待处理事项时随时可用
  function renderActionConsole() {
    ...
    if (!pa || state.phase==='gameover') {
      if (state.phase==='gameover') {
        el.innerHTML = `<div class="ac-idle"><div class="ac-gameover">🏁 游戏结束</div></div>`;
        return;
      }
      ...
      if (isMyTurn) {
        ...
        const moveHint = state.movesLeft > 0
          ? `<div class="ac-moves-info">
              <span class="ac-moves-icon">👣</span>
              <span>剩余步数 <b>${state.movesLeft}</b> 步</span>
             </div>
             <div class="ac-hint">点击绿色格子移动，也可使用楼梯</div>`
          : `<div class="ac-moves-info moves-done">
              <span class="ac-moves-icon">✓</span>
              <span>本回合移动步数已用完</span>
             </div>`;

        const attackSection = attacks.length
          ? `<div class="ac-attack-section">
              <div class="ac-attack-label">⚔️ 可攻击目标（同房间）：</div>
              ${attacks.map(tid=>{
                const t=state.players.find(p=>p.id===tid);
                return t ? `<button class="btn-attack" onclick="Game.attack('${tid}')">
                  ⚔️ 攻击 ${t.char?.emoji||''} <b>${t.name}</b>（力量${t.stats?.might}）
                </button>` : '';
              }).join('')}
            </div>`
          : state.phase==='haunt' && state.hasAttackedThisTurn
            ? `<div class="ac-attacked-note">✓ 本回合已使用攻击</div>`
            : state.phase==='haunt'
              ? `<div class="ac-hint-dim">⚔️ 需与目标在同一房间才能攻击</div>`
              : `<div class="ac-hint-dim">🩸 鬼魂降临后可发起攻击</div>`;

        el.innerHTML = `<div class="ac-active-turn">
          <div class="ac-title-turn">⭐ 你的回合</div>
          ${moveHint}
          ${attackSection}
          ${rollDisplay}
          <button class="btn-end-turn" onclick="Game.endTurn()">✓ 结束回合</button>
        </div>`;
      } else {
        // 等待其他玩家
        const fl={ground:'地面层',upper:'二楼',basement:'地下室'}[curPlayer?.floor]||'';
        el.innerHTML = `<div class="ac-waiting">
          <div class="ac-wait-icon">⏳</div>
          <div class="ac-wait-name">${curPlayer?.char?.emoji||'👤'} <b>${curPlayer?.name||'?'}</b> 的回合</div>
          <div class="ac-wait-detail">
            剩余步数：<b>${state.movesLeft}</b> 步 ·
            位置：${fl}${state.hasAttackedThisTurn?' · 已攻击':''}
          </div>
        </div>`;
      }
      return;
    ...
    if (pa.type === 'draw_card') {
      ...
      const note = pa.cardType==='omen'
        ? '<div class="ac-omen-note">🩸 摸牌后将进行凶兆检定，之后回合自动结束</div>'
        : '<div class="ac-note">📋 摸牌后本回合自动结束</div>';
      el.innerHTML = `<div class="ac-draw-request">
        <div class="ac-title">${typeIcon} ${actPlayer?.name||'?'} — 摸${typeLabel}牌</div>
        <div class="ac-subtitle">进入【${pa.roomName||'?'}】，须翻开一张${typeLabel}牌</div>
        ${note}
        ${isMyAction
          ? `<button class="btn-draw btn-${pa.cardType}" onclick="Game.drawCard()">🃏 翻牌！</button>`
          : `<div class="ac-wait-other">等待 ${actPlayer?.name||'?'} 翻牌…</div>`}
      </div>`;
    }

    // 属性检定
    else if (pa.type === 'roll_check') {
      const card = pa.card || state.currentCard;
      const matchedIdx = state.lastRoll?.matchedOutcomeIdx;
      const effLabel = e => {
        if (e.type==='stat') return `${STAT_NAME[e.stat]||e.stat}${e.val>0?'+':''}${e.val}`;
        if (e.type==='teleport') return `传送至${e.floor||''}`;
        if (e.type==='next_turn_bonus') return `下回合+${e.steps}步`;
        if (e.type==='explore_move') return `探索${e.floor||''}层`;
        return '';
      };
      const tiersHtml = pa.outcomes
        ? `<div class="ac-tiers">${pa.outcomes.map((o,i)=>{
            const isMatched = matchedIdx !== undefined && matchedIdx === i;
            const eff = (o.effects||[]).map(effLabel).filter(Boolean).join('，') || '无效果';
            return `<div class="ac-tier${isMatched?' matched':''}">
              <span class="tier-label">${o.label}</span>
              <span class="tier-body">
                <span class="tier-desc">${o.desc}</span>
                <span class="tier-eff">${eff}</span>
              </span>
            </div>`;
          }).join('')}</div>`
        : '';

      const afterNote = pa.isOmenCard
        ? '<div class="ac-omen-note">🩸 检定后将进行凶兆检定（固定掷6枚骰子）</div>'
        : '<div class="ac-note">📋 检定后本回合自动结束</div>';

      el.innerHTML = `<div class="ac-check">
        <div class="ac-title">🎲 ${STAT_NAME[pa.checkStat]||pa.checkStat}检定${pa.isOmenCard?' 🩸':''}
          — 掷 <b>${pa.diceCount}</b> 枚骰</div>
        ${card ? `<div class="ac-card-inline">
          <div class="ac-card-name">${card.name}</div>
          <div class="ac-card-flavor">"${card.flavor||card.desc||''}"</div>
        </div>` : ''}
        ${tiersHtml}
        ${rollDisplay}
        ${afterNote}
        ${isMyAction
          ? `<button class="btn-dice" onclick="Game.rollDice()" ${diceRollInProgress?'disabled':''}>
              🎲 掷${STAT_NAME[pa.checkStat]}骰！</button>`
          : `<div class="ac-wait-other">等待 ${actPlayer?.name||'?'} 掷骰…</div>`}
      </div>`;
    }

    // 凶兆检定（固定6枚骰子）
    else if (pa.type === 'roll_haunt') {
      el.innerHTML = `<div class="ac-haunt-check">
        <div class="ac-title">🩸 凶兆检定！</div>
        <div class="ac-subtitle">固定掷 <b>6</b> 枚骰子，已积累 <b>${state.omenCount}</b> 个凶兆</div>
        <div class="ac-check-rule">
          若结果 <b class="danger">小于 ${state.omenCount}</b>，幽灵降临！
          若结果 <b class="success">≥ ${state.omenCount}</b>，暂时安全。
        </div>
        <div class="ac-note">📋 检定后本回合自动结束</div>
        ${rollDisplay}
        ${isMyAction
          ? `<button class="btn-dice btn-haunt" onclick="Game.rollDice()" ${diceRollInProgress?'disabled':''}>🎲 进行凶兆检定！</button>`
          : `<div class="ac-wait-other">等待 ${actPlayer?.name||'?'} 掷骰…</div>`}
      </div>`;
    }
    ...
    // 攻击方掷骰
      ...
      const atk = state.players.find(p=>p.id===pa.attackerId);
      const def = state.players.find(p=>p.id===pa.defenderId);
      el.innerHTML = `<div class="ac-combat">
        <div class="ac-title">⚔️ 战斗 — 攻击方掷骰</div>
        <div class="ac-combat-matchup">
          <span style="color:${atk?.char?.color||'#fff'}">${atk?.char?.emoji||''} ${atk?.name||'?'}</span>
          <span class="vs">⚔️</span>
          <span style="color:${def?.char?.color||'#fff'}">${def?.char?.emoji||''} ${def?.name||'?'}</span>
        </div>
        <div class="ac-subtitle"><b>${atk?.name||'?'}</b> 掷力量骰（${pa.diceCount}枚），数值越高越好</div>
        <div class="ac-combat-rule">攻方总数 − 守方总数 = 守方受到的伤害（减少力量）</div>
        ${rollDisplay}
        ${isMyAction
          ? `<button class="btn-dice btn-attack-dice" onclick="Game.rollDice()" ${diceRollInProgress?'disabled':''}>🎲 掷攻击骰！</button>`
          : `<div class="ac-wait-other">等待 ${atk?.name||'?'} 掷骰…</div>`}
      </div>`;
    }
    ...
    // 防御方掷骰
      ...
      const def = state.players.find(p=>p.id===pa.defenderId);
      el.innerHTML = `<div class="ac-combat">
        <div class="ac-title">🛡 战斗 — 防御方掷骰</div>
        <div class="ac-combat-matchup">
          <span style="color:${atk?.char?.color||'#fff'}">${atk?.char?.emoji||''} 攻击值 = <b>${pa.atkTotal}</b></span>
        </div>
        <div class="ac-subtitle"><b>${def?.name||'?'}</b> 掷力量骰（${pa.diceCount}枚），数值越高越好</div>
        <div class="ac-combat-rule">防御总数 ≥ 攻击值 → 防御成功；否则受到差值伤害（减少力量）</div>
        ${rollDisplay}
        ${isMyAction
          ? `<button class="btn-dice btn-defend-dice" onclick="Game.rollDice()" ${diceRollInProgress?'disabled':''}>🎲 掷防御骰！</button>`
          : `<div class="ac-wait-other">等待 ${def?.name||'?'} 掷骰…</div>`}
      </div>`;
    }
  }
  ...
  function renderDiceResult(roll) {
    ...
    const diceHtml = roll.dice.map(d =>
      `<div class="die die-${d}">${d}</div>`
    ).join('');
    return `<div class="dice-result">
      <div class="dice-row">${diceHtml}</div>
      <div class="dice-total">总计 <b>${roll.total}</b></div>
    </div>`;
  }
  ...
  function renderCardHistory() {
    ...
    el.innerHTML = cards.slice().reverse().map(c => {
      ...
      return `<div class="history-card history-${c.type}" onclick="Game.showCardById('${c.id}')">
        <span class="hc-icon">${typeIcon}</span>
        <span class="hc-name">${c.name}</span>
        <span class="hc-who">${drawerName}</span>
      </div>`;
    }).join('');
  }
  ...
  function renderLog() {
    ...
    el.innerHTML = state.log.map(e =>
      `<div class="log-entry log-${e.type||'normal'}">${e.text}</div>`
    ).join('');
  ...
  function showCard(card) {
    ...
    const effLabel = e => {
      if (e.type==='stat') return `${sn[e.stat]||e.stat}${e.val>0?'+':''}${e.val}`;
      if (e.type==='teleport') return `传送至${e.floor||''}${e.roomId||''}`;
      if (e.type==='next_turn_bonus') return `下回合+${e.steps}步移动`;
      if (e.type==='explore_move') return `探索${e.floor||''}层`;
      if (e.stat) return `${sn[e.stat]||e.stat}${e.val>0?'+':''}${e.val}`;
      return '';
    };
    let effectHtml = '';
    if (card.outcomes) {
      const tiersHtml = card.outcomes.map(o => {
        const eff = (o.effects||[]).map(effLabel).filter(Boolean).join('，') || '无效果';
        return `<div class="modal-tier">
          <span class="modal-tier-label">${o.label}</span>
          <div class="modal-tier-content">
            <div class="modal-tier-desc">${o.desc}</div>
            <div class="modal-tier-eff">${eff}</div>
          </div>
        </div>`;
      }).join('');
      effectHtml = `<div class="modal-check">
        <div class="modal-check-header">
          【${sn[card.checkStat]||card.checkStat}检定】掷当前属性值数量的骰子
        </div>
        <div class="modal-tiers">${tiersHtml}</div>
        ${card.type==='omen'?'<div class="modal-omen-note">🩸 检定后还需进行凶兆检定（掷6枚骰子）</div>':''}
      </div>`;
  ...
  function showCardById(id) {
    const card = state.drawnCards?.find(c=>c.id===id);
